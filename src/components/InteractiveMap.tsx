...
function createColoredTileLayer(taxonKey: number, color: string, style: string, extraParams: string) {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < resolvedKeys.length; i++) {
      ...
      if (key) {
        ...
        const LayerClass = createColoredTileLayer(key, color, tileMode.style, tileMode.params)
        // eslint-disable-next-line @typescript-eslint/no-unsafe-argument
        const layer = new (LayerClass as unknown as new (opts: L.GridLayerOptions) => L.GridLayer)({ opacity: 0.7, maxZoom: 10 }).addTo(map)
